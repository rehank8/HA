...
namespace HA.Services
{
	public class AccountService
	{
		...
		public List<UserIndex> GetVendorslist()
		{
			...
			{
				throw new ApplicationException("Could not get vendors details");
			}
			return null;
		}
		...
		public List<EndUserAppointment> GetUserAppointments()
		{
			client.DefaultRequestHeaders.Add("Authorization", Helper.LoginResponse.token_type + " " + Helper.LoginResponse.access_token);
			var response = client.GetAsync(enduserappointments).Result;
			if (response.StatusCode == HttpStatusCode.OK)
			{
				var result = response.Content.ReadAsStringAsync().Result;
				List<EndUserAppointment> appointments = JsonConvert.DeserializeObject<List<EndUserAppointment>>(result);
				return appointments;
